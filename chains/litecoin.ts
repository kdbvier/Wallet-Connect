  ...
  * 
  */
 export default class LiteCoinLogic extends CoinInterface {
      ...
      */
     constructor(config: LiteCoinLogicConfig) {
         super();
      ...
      * @param config 
      */
     setConfig(config: LiteCoinLogicConfig) {
     ...
     async getBalance(address: string): Promise<number | string | Error> {
         return new Promise(async (resolve, reject) => {
             try {
                 ...
                 let url =  `https://api.cryptoapis.io/v1/bc/ltc/${networkPart}/address/${address}`;
     ...
     async transferFunds(senderAddress: string, senderPrivateKey: string, receiverAddress: string, amount: number): Promise<string> {
         return new Promise(async (resolve, reject) => {
             try {
                 let networkPart = this.network ==='testnet' ? 'testnet': 'mainnet';
                 let url =  `https://api.cryptoapis.io/v1/bc/ltc/${networkPart}/address/${senderAddress}`;
                 let response = await axios.get(url, {headers: {"X-API-Key": apiKey}});
                 let balance = response.data.payload.balance;
                 if (balance < amount) {
                 ...
                 //Get transaction fee
                 let fee_url = `https://api.cryptoapis.io/v1/bc/ltc/${networkPart}/txs/fee`;
                 ...
                 let create_url =  `https://api.cryptoapis.io/v1/bc/ltc/${networkPart}/txs/create`;
                 let data = {
                     inputs: 
                 ...
                 let sign_url =  `https://api.cryptoapis.io/v1/bc/ltc/${networkPart}/txs/sign`;
                 let sign_data = {
                     hex: prepared,
                 ...
                 //Send transaction
                 let send_url =  `https://api.cryptoapis.io/v1/bc/ltc/${networkPart}/txs/send`;
                 let tx_data = {
                     hex: sign_hex
                 }
