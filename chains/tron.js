...
export default class TronLogic extends CoinInterface {
    ...
    async getBalance(address) {
        return new Promise((resolve, reject) => {
            this.tronWeb.trx.getBalance(address).then((result) => {
                resolve(result)
     ...
     */
    async transferFunds(senderAddress, senderPrivateKey, receiverAddress, amount) {
        return new Promise(async (resolve, reject) => {
            ...
            const transaction = await this.tronWeb.trx.sendTransaction(receiverAddress, amount * 10 ** 6, senderPrivateKey)
