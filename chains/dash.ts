import CoinInterface from "../CoinInterface";
  ...
  * 
  */
 export default class DashLogic extends CoinInterface {
      ...
      */
     constructor(config: DashLogicConfig) {
         super();
      ...
      * @param config 
      */
     setConfig(config: DashLogicConfig) {
     ...
     async getBalance(address: string): Promise<number | string | Error> {
         return new Promise(async (resolve, reject) => {
             try {
                 ...
                 let url =  `https://api.cryptoapis.io/v1/bc/dash/${networkPart}/address/${address}`;
     ...
     async transferFunds(senderAddress: string, senderPrivateKey: string, receiverAddress: string, amount: number): Promise<string> {
         return new Promise(async (resolve, reject) => {
             try {
                 let networkPart = this.network ==='testnet' ? 'testnet': 'mainnet';
                 let url =  `https://api.cryptoapis.io/v1/bc/dash/${networkPart}/address/${senderAddress}`;
                 let response = await axios.get(url, {headers: {"X-API-Key": apiKey}});
                 let balance = response.data.payload.balance;
                 if (balance < amount) {
                 ...
                 //Get transaction fee
                 let fee_url = `https://api.cryptoapis.io/v1/bc/dash/${networkPart}/txs/fee`;
                 ...
                 let create_url =  `https://api.cryptoapis.io/v1/bc/dash/${networkPart}/txs/create`;
                 let data = {
                     inputs: 
                 ...
                 let sign_url =  `https://api.cryptoapis.io/v1/bc/dash/${networkPart}/txs/sign`;
                 let sign_data = {
                     hex: prepared,
                 ...
                 //Send transaction
                 let send_url =  `https://api.cryptoapis.io/v1/bc/dash/${networkPart}/txs/send`;
                 let tx_data = {
                     hex: sign_hex
                 }
