...
const apiKey = '18e4aa9e0db276a0d010aedc8bd3d643baca4889';
 ...
 * 
 */
export default class DogeCoinLogic extends CoinInterface {
    private networkEndPoint: string = DEFAULT_NETWORK;
    private network: string = DEFAULT_NETWORK;
    // private api = new dogecoin.Client(this.network);
    /**
     * Constructor of XRPLogic class
     * @param config network configuration
     */
    constructor(config: DogeCoinLogicConfig) {
        super();
     ...
     * @param config 
     */
    setConfig(config: DogeCoinLogicConfig) {
    ...
    async getBalance(address: string): Promise<number | string | Error> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let url =  `https://api.cryptoapis.io/v1/bc/doge/${networkPart}/address/${address}`;
    ...
    async transferFunds(senderAddress: string, senderPrivateKey: string, receiverAddress: string, amount: number): Promise<string> {
        return new Promise(async (resolve, reject) => {
            try {
                let networkPart = this.network ==='testnet' ? 'testnet': 'mainnet';
                let url =  `https://api.cryptoapis.io/v1/bc/doge/${networkPart}/address/${senderAddress}`;
                let response = await axios.get(url, {headers: {"X-API-Key": apiKey}});
                let balance = response.data.payload.balance;
                if (balance < amount) {
                ...
                //Get transaction fee
                let fee_url = `https://api.cryptoapis.io/v1/bc/doge/${networkPart}/txs/fee`;
                ...
                let create_url =  `https://api.cryptoapis.io/v1/bc/doge/${networkPart}/txs/create`;
                let data = {
                    inputs: 
                ...
                let sign_url =  `https://api.cryptoapis.io/v1/bc/doge/${networkPart}/txs/sign`;
                let sign_data = {
                    hex: prepared,
                ...
                //Send transaction
                let send_url =  `https://api.cryptoapis.io/v1/bc/doge/${networkPart}/txs/send`;
                let tx_data = {
                    hex: sign_hex
                }
